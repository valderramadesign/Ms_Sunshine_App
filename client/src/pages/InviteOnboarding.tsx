...
type InviteInfo =
  | { valid: true; email: string; role: Role; invitedByName: string; schoolName: string }
  | { valid: false };

export const InviteOnboarding = (): JSX.Element => {
  ...
  const { data, isLoading } = useQuery<InviteInfo>({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiRequest("POST", `/api/invitations/${params.token}/accept`, { password });
      ...
      if (msg.includes("409")) {
        setErrors({ general: "An account already exists for this email. Please log in." });
      } else {
        setErrors({ general: "Could not complete setup. The invitation may have expired." });
      }
    } finally {
      setSubmitting(false);
    }
  ...
  if (isLoading) {
    return <div className="fixed inset-0 flex items-center justify-center bg-[#f5f5f5]" data-testid="invite-loading" />;
  }
  ...
  if (!data || !data.valid) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center bg-[#f5f5f5] px-[24px] text-center gap-[20px]">
        <p
          data-testid="text-invite-invalid"
        ...
        <button
          type="button"
          data-testid="button-invite-go-login"
          onClick={() => setLocation("/")}
    ...
    <div className="fixed inset-0 flex flex-col items-center bg-[#f5f5f5] px-[24px] overflow-y-auto pt-[10dvh] pb-[10dvh]">
      <div className="flex flex-col gap-[32px] items-center w-full max-w-[402px] my-auto">
        <div className="flex flex-col gap-[8px] items-center text-center">
          <p
            data-testid="text-invite-welcome"
          ...
          <p
            data-testid="text-invite-subtitle"
          ...
          >
            {data.invitedByName ? `${data.invitedByName} invited you to ` : "You've been invited to "}
            {data.schoolName}
            {data.role === "teacher" ? " as a teacher." : "."}
        ...
        <div className="flex flex-col gap-[20px] items-start w-full">
          <div className="flex flex-col gap-[4px] items-start w-full">
            ...
            <input
              ...
              readOnly
              disabled
              className={`${inputClass("email")} opacity-70`}
            />
          </div>
          ...
          <div className="flex flex-col gap-[4px] items-start w-full">
            ...
            <div className="relative w-full">
              ...
              >
                <img src="/figmaAssets/icon-eye.png" alt="toggle password" className="w-[18px] h-[13px] object-contain" style={{ opacity: showPassword ? 1 : 0.5 }} />
              </button>
            </div>
            {errors.password && (
              <p data-testid="error-invite-password" className="text-[#b34d3b] text-[12px] [font-family:'SF_Pro_Rounded-Regular','M_PLUS_Rounded_1c',Helvetica]">
                {errors.password}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-[4px] items-start w-full">
            ...
            />
            {errors.confirm && (
              <p data-testid="error-invite-confirm" className="text-[#b34d3b] text-[12px] [font-family:'SF_Pro_Rounded-Regular','M_PLUS_Rounded_1c',Helvetica]">
                {errors.confirm}
              </p>
            )}
            {errors.general && (
              <p data-testid="error-invite-general" className="text-[#b34d3b] text-[12px] [font-family:'SF_Pro_Rounded-Regular','M_PLUS_Rounded_1c',Helvetica]">
                {errors.general}
              </p>
            )}
          </div>
